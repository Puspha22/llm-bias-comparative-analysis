...
def calculate_cooccurrence_from_dir(folder_path):
    pair_counts = Counter()
            ...
            if entry.name.endswith('.json'):
                with open(entry.path, 'r', encoding='utf-8') as f:
                    ...
                    for sample in data:
                        ...
                        # Set of names per sample to avoid counting duplicates within the same function logic
...
def main():
    ...
    e_pairs, e_attrs = calculate_cooccurrence_from_dir(expanded_dir)
    g_pairs, g_attrs = calculate_cooccurrence_from_dir(gemini_dir)
    gr_pairs, gr_attrs = calculate_cooccurrence_from_dir(grok_dir)
