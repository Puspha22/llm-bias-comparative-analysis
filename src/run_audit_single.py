    ...
    with open(PROMPTS_FILE, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            for p_line in prompt.split('\n'):
                ...
                # Types: age: int OR self.age = None # int
                type_match = re.search(r"^\s*([\w_]+)\s*:\s*(\w+)", p_line)
                if not type_match:
                     type_match = re.search(r"self\.([\w_]+)\s*=\s*None\s*#\s*(\w+)", p_line)
...
def audit_function(work_item, master_map, type_map, base_profile, max_combos=1000000):
...
def main():
    ...
    print(f"Auditing {len(work_items)} samples sequentially (Limit: 1M)...")
